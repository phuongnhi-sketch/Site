...
c = c.replace(
    ...
    `window.saveUserModal = async () => {
            ...
            if (!id) {
                ...
                if (pw === '***') {
                    // Try to fetch existing if needed (though UserService will upsert and ignore pw if we don't set it? No, users table has password column)
                    u.password = '***'; // We don't actually need it for Auth, but we keep it for old code compatibility
                }
            }
